...
class MusicDataAnalyzer:
    ...
    def _parse_billboard_content(self, content: str) -> Dict:
        ...
        # 解析热门歌曲TOP10
        top_songs_section = self._extract_section(content, "## 爆款金曲榜 TOP10", "## 黑马榜")
        if top_songs_section:
            data['top_songs'] = self._parse_top_songs_table(top_songs_section)
        
        # 解析黑马榜
        rising_songs_section = self._extract_section(content, "## 黑马榜", "## 谁在点歌")
        if rising_songs_section:
            data['rising_songs'] = self._parse_rising_songs_table(rising_songs_section)
        
        # 解析用户画像
        user_section = self._extract_section(content, "## 谁在点歌：用户画像与地域偏好", "## 什么时间点歌最多")
        if user_section:
            data['user_demographics'] = self._parse_user_demographics(user_section)
            data['regional_preferences'] = self._parse_regional_preferences(user_section)
        
        # 解析时间分析
        time_section = self._extract_section(content, "## 什么时间点歌最多", "## 音乐标签风向标")
        if time_section:
            data['time_analysis'] = self._parse_time_analysis(time_section)
        
        # 解析标签趋势
        tag_section = self._extract_section(content, "## 音乐标签风向标", "## 下一首爆红歌曲预测")
        if tag_section:
            data['tag_trends'] = self._parse_tag_trends(tag_section)
        
        # 解析DJ榜单
        dj_section = self._extract_section(content, "## DJ 榜单 Top10", "## 出品机构与版权声明")
        if dj_section:
    ...
    def _parse_user_demographics(self, section: str) -> Dict:
        ...
        if gender_match:
            ...
            demographics['gender'] = {'male': male_percent, 'female': 100 - male_percent}
            ...
            if '| 城市类型' in line:
                ...
                if len(parts) >= 4:
                    try:
                        region = {
                            'city_type': parts[1],
                            'preferred_tags': [tag.strip() for tag in parts[2].split('、')],
                            'typical_songs': [song.strip() for song in parts[3].split('、')]
                        }
    ...
    def analyze_user_demographics(self) -> Dict:
        """分析用户画像数据"""
        return self.data.get('user_demographics', {
            'gender': {'male': 58, 'female': 42},
            'age_groups': {
                '18-24': 34,
                '25-34': 29,
    ...
    def analyze_regional_trends(self) -> List[Dict]:
        """分析地域偏好趋势"""
        return self.data.get('regional_preferences', [
            {
                'city_type': '一线城市',
    ...
    def _save_structured_data(self):
        ...
        if self.data.get('user_demographics'):
            with open(f'{processed_dir}/user_demographics.json', 'w', encoding='utf-8') as f:
                json.dump(self.data['user_demographics'], f, ensure_ascii=False, indent=2)
        
        # 保存地域偏好数据
        if self.data.get('regional_preferences'):
            with open(f'{processed_dir}/regional_preferences.json', 'w', encoding='utf-8') as f:
                json.dump(self.data['regional_preferences'], f, ensure_ascii=False, indent=2)
        
        # 保存时间分析数据
        if self.data.get('time_analysis'):
            with open(f'{processed_dir}/time_analysis.json', 'w', encoding='utf-8') as f:
